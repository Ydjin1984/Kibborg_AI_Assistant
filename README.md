...
<p align="center">
  ...
  разбирает видео, считает рынок по настоящим свечам Binance.
</p>
...
<p align="center">
  <img src="docs/images/web-chat.png" alt="Веб-панель Kibborg: чат, статус мозга, Chrome и голос">
</p>
...
| Разбор тикера | `/analyze BTC` или вкладка «Анализ» |
...
1. Откройте [http://127.0.0.1:8090](http://127.0.0.1:8090) — в шапке чип «мозг» должен стать зелёным.
...
| Ключ | Пример | Смысл |
|---|---|---|
| `MODEL_PATH` | `models\brain\Qwen3.6-35B-A3B\....gguf` | какой GGUF грузить |
...
|---|---|
| 💬 Чат | обычные сообщения, вложения, 🎙 |
| 📊 Анализ | `/analyze` |
| 🛠 Агент | `/agent` / `/browser` |
...
- 🗜 Сжать = `/compact`;
- Сброс = `/reset`;
- полоса контекста — сколько токенов уже занято.
...
Если видеокарта одна и меньше 20 ГБ — поставьте модель поменьше (Qwen 8B / 14B GGUF) и пропишите новый `MODEL_PATH`. Агент будет слабее держать JSON диспетчера и длинные цепочки, но чат заработает.
