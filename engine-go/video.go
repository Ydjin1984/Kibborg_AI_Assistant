...
const framePrompt = `Опиши этот кадр из видео по делу, без выдумок: что происходит, какой интерфейс или сцена видны.
ЕСЛИ на кадре есть ТЕКСТ (код, слайд, заголовок, название, подпись, адрес) — выпиши его ДОСЛОВНО, отдельной строкой «ТЕКСТ НА ЭКРАНЕ: …».
Если кадр пустой, чёрный или размытый — так и скажи одной строкой.`
...
//
// Смысл в том, чтобы не платить зрением за пустоту и не оставлять пустым разбор:
//   - речи нет вообще (немой скринкаст) — кадры единственный источник, берём шесть;
//   - речь есть, ролик длинный — кадры почти ничего не добавят к расшифровке, берём ноль;
//   - короткий ролик с речью — три кадра дают сцену и обстановку задёшево.
