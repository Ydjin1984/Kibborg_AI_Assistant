...
6. Цены и суммы — только цифры, по-русски: пробел тысяч и запятая («63 087 $», «1 883,3 $»).
   Пропись НЕ пиши: её один раз добавит движок. Иначе озвучка повторяет число трижды.
7. Источник — «Источник: Интерфакс», не сырой URL в середине фразы. Ссылку можно в скобках после имени.

Формат ответа на новости/факты: список пунктов «- **Факт** … Источник: [имя](url)».
