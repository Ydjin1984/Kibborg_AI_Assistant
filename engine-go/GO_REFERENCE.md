...
├── document.go             # слой документов: PDF → текстовый слой / OCR скана / зрение
├── summarize.go            # общая карта-свёртка длинного текста (расшифровки, документы)
...
│   ├── gerchik.go          # алгоритм Герчика на D1: уровни, ATR H−L, ЛП/отбой/пробой
...
Рабочие правила слойного агента — `AGENTS.md`.
...
### Разбор по Герчику (2026-08-13)

`trading/gerchik.go` — вторая методика поверх тех же свечей Binance, отрисовка в
`gerchik_render.go`, дневные бары тянет `fetchDailyBars` (220 свечей: 180 дней истории по
§2.1 плюс запас на EMA-200 и текущий незакрытый день). Считается независимо от скоринга и
кладётся в `DecisionReport.Meta["gerchik"]`, поэтому Telegram, веб-панель и инструмент
`analyze_ticker` получают её одинаково — все три идут через `renderReport`.

Что оказалось важным при проверке на живом BTC (каждый пункт — исправленный дефект):

- **ATR только по методике.** `High−Low` трёх нормальных дней, паранормальные (≥1.5 и ≤0.5
  базы) выброшены. Wilder-ATR из `indicators.go` тут не годится — другое число и другой
  стоп; тест это специально сторожит.
- **Касание — это экстремум У уровня**, а не любой бар, накрывший его телом. Со вторым
  определением проход цены через зону давал рядовому уровню «30 касаний» и силу, которой
  у него нет.
- **Вклад касаний ограничен** (`maxTouchBonus`): иерархия §2.2 ставит тип уровня выше их
  числа, иначе уровень, вдоль которого цена простояла два месяца, вытесняет всю разметку.
- **Разметка обязана быть по обе стороны цены** (`pickLevels`): отбор «топ-5 по силе» выдал
  пять сопротивлений подряд, и сценарий не строился в принципе — ни цели, ни поддержки.
  Плюс минимальный разнос `minLevelGapATR`, иначе пятёрка садится в одну зону.
- **Зеркальность — смена РОЛИ** (`mirrorRole`: серии закрытий по обе стороны + два перехода),
  а не просто закрытия выше и ниже: последнее верно для любого уровня внутри диапазона, и
  «зеркальными» становились все подряд. Аналогично ЛП: значим повторный или свежий.
- **Цели частями 3:1 / 4:1 / 5:1, не дальше следующего уровня** (§8). Одна цель «до уровня»
  давала на живом BTC RR 1:31 — арифметику, а не торговый план.
- **Заготовка ≠ сигнал.** Пока цена не подошла к уровню (`nearLevelATR`), сценарий считается,
  но помечается `Pending` и заголовком «Заготовка», а причина лежит в `Blocks`.

Промпты знают об этом блоке: `narrateSystemPrompt` требует объяснять и его, а строки «методика
запрещает вход» пересказывать как отказ; правило пака `trade` в `agent_loop.go` запрещает
предлагать вход вопреки им.

**Модели входа** (`gerchik_setup.go`, `detectModel`) разбирают поведение последних баров у
уровня: ЛП одним баром (прокол и возврат), ЛП двумя барами (пробойный ЗАКРЫЛСЯ за уровнем,
следующий вернулся), ЛП тремя и больше, те же модели «в работе» (цена сидит за уровнем прямо
сейчас — ждём возврата), отбой БСУ-БПУ с проверкой «БПУ2 не ломает БПУ1» и пробой. Вся
зеркальность лонга и шорта живёт в одном знаке `z` внутри `detectModel` — правила курса для
обеих сторон одинаковы, и дублировать их значило бы плодить расхождения.

Три вещи, на которых модели ломались при проверке:

- **порядок распознавания.** Если сперва проверять прокол последнего бара, любой сложный ЛП
  схлопывается в «ЛП 1 баром» — сначала считается, сколько баров ДО последнего сидели за
  уровнем;
- **«бар в зоне» — это закрытие за уровнем без допусков** (методика говорит про закрытие);
  лишний допуск съедал модели на 2 и 3 барах;
- **прокол, наоборот, требует порога** (`lpPierceATR`): уровень собран с допуском
  `levelTolATR`, иначе «ложным пробоем» становится любое касание хвостом.

Пробой (§6.5) берётся только по тренду, на маленьких барах и с входом ЗА уровень — это
единственная модель, где сделка идёт сквозь уровень, а не от него.

Чего в слое пока НЕТ (чтобы не считать сделанным то, что не сделано):

- субъективные предпосылки (§5) — дистрибуция, зона заражённости, задёрг — не считаются и
  не выдумываются; формализуемых сейчас 14;
- фандинг (§10) в издержки не входит: он зависит от времени удержания, а его методика не
  задаёт. Комиссия обеих сторон и проскальзывание учтены в `RRNet`;
- дисциплина исполнения (3 сделки в день, 3 убытка подряд, EOD, отмена ордера по двум
  стопам) не реализована — движок анализирует, но не исполняет; журнал ведётся отдельно
  (`journal_add`);
- «пустая зона» (§2.4) формализована как отсутствие других уровней между ценой и целью;
  более тонкое чтение графика сюда не заложено.
...
### PDF и сканы: три источника текста, порядок важнее инструментов (2026-08-13)

Тот же приём, что с видео: PDF — контейнер. `pdftotext` берёт текстовый слой одним вызовом и
режет его по страницам символом `\f`; страницы, где текста меньше `pdfPageTextMin`, считаются
картинками и уходят в `pdftoppm` (300 dpi) → `tesseract` (rus+eng). Зрение подключается только
там, где tesseract вернул пустоту, и не более чем на `pdfVisionRetryPages` страниц.

Зрение стоит ПОСЛЕДНИМ намеренно. Оно понимает вёрстку лучше, но возвращает пересказ, а не
символы: для акта, договора или справки «улучшенный» номер документа — это порча. Tesseract
буквален, поэтому он основной, а зрение — запасной.

Три вещи, измеренные на этой машине:

- **`runTool` склеивал stderr со stdout.** На битом PDF строки `Syntax Error: …` от poppler
  попадали прямо в текст документа и уходили в выжимку как его содержание. Появился
  `runToolOut` — только stdout; туда же переведён разбор JSON от ffprobe, где склейка потоков
  сломала бы парсинг ровно так же.
- **Молчаливая подмена языка OCR.** Без `rus.traineddata` tesseract не отказывает, а выдаёт
  правдоподобную латиницу: «AKT npnéma-nepezaun» вместо «АКТ приёма-передачи». Каталог языков
  теперь ищется по настройке → cwd → рядом с exe, использованные языки печатаются в отчёте, а
  отсутствие русского — отдельное предупреждение.
- **winget правит PATH ПОЛЬЗОВАТЕЛЯ, и живые процессы этого не видят.** Свежепоставленный
  poppler «не найден», хотя лежит на диске. `popplerExe` ищет и в `WinGet\Links`, и в самом
  каталоге пакета — как `tesseractExe` уже искал стандартную установку.

Проверка сделана на СИНТЕЗИРОВАННОМ скане: страница рисуется через System.Drawing, JPEG
вручную заворачивается в одностраничный PDF (`makescan.ps1`), после чего `pdftotext` на нём
возвращает ноль символов — то есть текстового слоя нет вовсе. На таком файле распознавание
русского акта 1998 года заняло 4 секунды и дало верный текст с двумя огрехами на изолированных
инициалах («Кузнецова A.B.» латиницей) — типичная для OCR путаница похожих букв.
